          ...
          BlocBuilder<SearchBloc, SearchState>(
            builder: (context, state) {
              if(state is EmptySearchState) {
                ...
                return MultiBlocProvider(
                  providers: [
                    BlocProvider<ComicVineIssuesBloc>(create: (context) => ComicVineIssuesBloc()..add(QueryComicVineIssuesEvent(5, query: state.query))),
                    BlocProvider<ComicVineMoviesBloc>(create: (context) => ComicVineMoviesBloc()..add(QueryComicVineMoviesEvent(5, query: state.query))),
                    BlocProvider<ComicVineSeriesListBloc>(create: (context) => ComicVineSeriesListBloc()..add(QueryComicVineSeriesListEvent(5, query: state.query))),
                  ], 
                  child: Builder(
                    builder: (context) {
                      final stateIssues = context.watch<ComicVineIssuesBloc>().state;
                      final stateMovies = context.watch<ComicVineMoviesBloc>().state;
                      final stateSeriesList = context.watch<ComicVineSeriesListBloc>().state;

                      if(stateIssues is LoadingComicVineIssuesState || stateMovies is LoadingComicVineMoviesState || stateSeriesList is LoadingComicVineSeriesListState) {
                        return Container(
                          alignment: Alignment.center,
                          child: const CircularProgressIndicator(),
                        );
                      }
                      else if(stateIssues is ErrorComicVineIssuesState || stateMovies is ErrorComicVineMoviesState || stateSeriesList is ErrorComicVineSeriesListState) {
                        return Container(
                          alignment: Alignment.center,
                              ...
                              children: <Widget>[
                                ...
                                BlocBuilder<ComicVineSeriesListBloc, ComicVineSeriesListState>(
                                  builder: (context, state) {
                                    ...
                                    else if(state is EmptyComicVineSeriesListState) {
                                  ...
                                  builder: (context, state) {
                                    if(state is LoadedComicVineIssuesState) {
                                      return CardSlider(
                                        ...
                                        hasButton: false, 
                                        title: "Comics",
                                        cardList: state.issues.map((e) => CardTemplate(imagePath: e.image?.smallUrl??'', description: e.name??'', onClick: (event) => context.go('/issue/${e.id}'))).toList(),  
                                      );
                                    }
                                    else if(state is EmptyComicVineIssuesState) {
                                      return const SizedBox(height: 0, width: 0,);
                                    }
                                    else {
                                      return Container(
                                ...
                                BlocBuilder<ComicVineMoviesBloc, ComicVineMoviesState>(
                                  builder: (context, state) {
                                    ...
                                    else if(state is EmptyComicVineMoviesState) {
                                      return const SizedBox(height: 0, width: 0,);
                                    }
                                    else {
                                      return Container(
...
class _SearchBar extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return AppSearchBar(onSubmitted: (query) => BlocProvider.of<SearchBloc>(context)..add(QuerySearchEvent(query: query)));
  }
}
