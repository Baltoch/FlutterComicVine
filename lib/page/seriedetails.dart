                      ...
                      Expanded(
                        child: Tabs(header: const [
                          ...
                          SingleChildScrollView(
                            ...
                            padding: const EdgeInsets.only(top: 30),
                            child: Column(
                              children: state.series.episodes!
                                  .map((e) => BlocProvider<
                                          ComicVineEpisodeBloc>(
                                      create: (context) =>
                                          ComicVineEpisodeBloc(e.id!)
                                            ..add(LoadComicVineEpisodeEvent()),
                                      child: BlocBuilder<ComicVineEpisodeBloc,
                                              ComicVineEpisodeState>(
                                          builder: (context, state) {
                                        if (state
                                            is LoadedComicVineEpisodeState) {
                                          return Container(
                                              padding: const EdgeInsets.only(
                                                  left: 16,
                                                  right: 16,
                                                  bottom: 15),
                                              child: Episode(
                                                imageURL: state.episode.image
                                                        ?.smallUrl ??
                                                    '',
                                                onClick: (event) => (),
                                                content: state.episode
                                                            .episodeNumber ==
                                                        null
                                                    ? ''
